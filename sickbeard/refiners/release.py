...
def refine(video, release_name=None, release_file=None, extension='release', **kwargs):
    ...
    logger.log(u'Guessing using <{0}>'.format(release_path), logger.DEBUG)
...
def get_release_file(dirpath, extension, fileroot):
    ...
    if os.path.isfile(release_file):
        logger.log(u'Found release file <{0}>'.format(release_file), logger.DEBUG)
        return release_file
...
def get_release_name(release_file):
    ...
    if not release_name:
        logger.log(u'Release file <{0}> does not contain a release name'.format(release_file), logger.WARNING)
